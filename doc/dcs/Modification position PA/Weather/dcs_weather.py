...
# real time / weather update script for dcs world (tested under 1.5/2.5.x)
# created by havoc-company.com
s_version_info = " V1.4.3 2020.JUN.18_10:35"
...
class G:  # Globals class ...... to store all the data we need to mess with (populate with some defaults)
    ...
    # you need to register with checkwx.com create an account, then request an api key (takes 24hrs or less)
    # once activated your key will appear in your API dashboard checkwx.com/user/profile
    # enter your key below between the quotes
    s_api_key_checkwx = '5c55a75eedbfe1fb1fe07dbcef'

    # you need to register with avwx.rest create an account, then request an api key
    # once activated your key will appear in your API dashboard account.avwx.rest/manage
    # enter your key below between the quotes
    s_api_key_avwx = 'gUUmsg9sWMxMZCHoEe1fSJVQFN9N79iH7aS_Fxu9TKY'
    ...
    l_dates = [['2020', '03', '24', '7', '30'], ['2020', '06', '21', '5', '50'], ['2020', '09', '17', '7', '45'],
               ['2020', '12', '14', '8', '30'], ['2020', '03', '24', '14', '0'], ['2020', '06', '21', '16', '0'],
               ['2020', '09', '17', '14', '0'], ['2020', '12', '14', '13', '0'],

               ['2020', '03', '24', '6', '10'], ['2020', '06', '21', '5', '0'], ['2020', '09', '17', '5', '0'],
               ['2020', '12', '14', '6', '30'], ['2020', '03', '24', '12', '0'], ['2020', '06', '21', '13', '0'],
               ['2020', '09', '17', '12', '0'], ['2020', '14', '14', '12', '0']
...
def check_weather_limits():
    ...
    if int(G.s_cloud_density) > int(G.s_cloud_density_max):  # this is to limit too much cloud cover > than 9 = total
        G.s_cloud_density = G.s_cloud_density_max

    if int(G.s_cloud_density) >= 9 or G.s_fog_enable == 'true' and int(G.s_cloud_base_m) <= 2000:
        G.s_cloud_base_m = '3000'
...
def convert_to_hr_and_min_to_seconds():  # DCS uses seconds from 00:00 as time of day (this converts hr&min to this)
    G.s_start_time = str((int(G.s_hour) - 6) * 3600 + (int(G.s_mins) * 60) + int(G.s_seconds))
...
def exit_script(code):
    print("weather.py", s_version_info, " exiting with code " + str(code))
    exit(code)
...
def gen_rand_dev(s_value, i_min, i_max):  # used to change values for wind direction at diff alts
...
def gen_rand_dev_speed(s_value, i_min, i_max):  # used to change values for wind speed at diff alts
    i_deviation = rd.randint(i_min, i_max)
    i_value = int(s_value) + i_deviation
    if i_value < 0:
        i_value = int(s_value)
...
def not_setup_correctly(s_server):
    print("")
    print("ERROR: You have not set-up your " + s_server + " API KEY YET")
    print("you need to register with " + s_server + "  create an account,")
    print("then request an api key (takes 24hrs or less)")
    print("once activated your key will appear in your API dashboard")
    print("enter your key in s_api_key_ variable near the top of this script")
    time.sleep(2)
...
def get_avwx_json_wind_direction(json_weather_l):
    ...
    i_tmp_dir = int(json_weather_l['wind_direction']['value'])
    i_tmp_dir = i_tmp_dir + 180  # invert direction due to DCS weirdness
    if i_tmp_dir >= 360:
        i_tmp_dir = i_tmp_dir - 360
    s_direction = str(i_tmp_dir)
...
def get_avwx_weather_wind(json_weather_l):  # read json weather data and set the wind info from it
    ...
    G.s_wind_speed_2k = gen_rand_dev_speed(s_mps, -2, 3)

    # wind at 8k - we use ground level value and randomise a bit more
    G.s_wind_speed_8k = gen_rand_dev_speed(s_mps, -2, 4)
...
def get_checkwx_weather_wind(json_weather_l):
    ...
    if s_direction is None:
        ...
        i_tmp_dir = int(json_weather_l['data'][0]['wind']['degrees'])
        i_tmp_dir = i_tmp_dir + 180  # invert direction due to DCS weirdness
        if i_tmp_dir >= 360:
            i_tmp_dir = i_tmp_dir - 360
        s_direction = str(i_tmp_dir)
    ...
    G.s_wind_dir_2k = gen_rand_dev(G.s_wind_dir_gnd, -10, 10)
    G.s_wind_dir_8k = gen_rand_dev(G.s_wind_dir_gnd, -20, 20)
    G.s_wind_speed_2k = gen_rand_dev_speed(G.s_wind_speed_gnd, -2, 3)
    G.s_wind_speed_8k = gen_rand_dev_speed(G.s_wind_speed_gnd, -2, 4)
...
# DCS mission file now no longer have this theatre value :-(
def is_sandstorm_terrain(l_theatre_id):
    s_persian_gulf = 'PersianGulf'
    if l_theatre_id[0] == s_persian_gulf:
        return True
    else:
        return False
...
def weather_read_url_checkwx():  # try to read metar data for airport(s) from internet
    ...
    s_url1 = 'https://api.checkwx.com/metar/' + G.s_primary_airport + '/decoded?pretty=1'
    s_url2 = 'https://api.checkwx.com/metar/' + G.s_backup_airport + '/decoded?pretty=1'
    ...
    # time.sleep(555)
    # import requests
    # hdr = {'X-API-Key': 'a8ac05ca850198a4a031305c5c'}
    # req = requests.get('https://api.checkwx.com/metar/EGAC/decoded?pretty=1', headers=hdr)
    # print(req.text)
...
def weather_read_url_avwx():  # try to read metar data for airport(s) from internet
    ...
    s_url1 = s_url_base + G.s_primary_airport + "?token=" + G.s_api_key_avwx
    s_url2 = s_url_base + G.s_backup_airport + "?token=" + G.s_api_key_avwx
    json_object = False  # just stuck this here to stick pycharm whining like a little bitch

    #  https://avwx.rest/api/metar/KJFK?token= YOUR TOKEN
    # we should try primary twice with a 5 second wait between attempts
    i_count = 0
    while i_count < 1:
...
# work out if it DUST enabled terrain
l_theatre = read_mission_file(G.s_mission_miz_path + 'theatre')
print(l_theatre)
G.b_sand_storm = is_sandstorm_terrain(l_theatre)
...
if G.b_update_weather:
    if G.b_debug_load:
        ...
        if G.s_api_key_checkwx == 'NOT_YET_SET':
            not_setup_correctly("checkwx.com")
        else:
            get_weather1()

        if G.json_weather is False:
            if G.s_api_key_avwx == 'NOT_YET_SET':
                not_setup_correctly("avwx.rest")
                exit_script(1)
            else:
                print("USING Secondary Server avxw")
                get_weather2()
